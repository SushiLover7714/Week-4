...
public class ExpenseTracker {
  ...
  public ExpenseTracker(
    ...
    this.user =
      new User(userDataFilePath, expenseRecordsFilePath, incomeRecordsFilePath);
  }
      ...
      for (int i = 0; i < uniqueCategoryList.size(); i++) {
        ...
        for (int j = 0; j < expenseRecords.size(); j++) {
          if (
            uniqueCategoryList
              .get(i)
              .equalsIgnoreCase(expenseRecords.get(j).getCategory())
          ) {
            categoryExpense += expenseRecords.get(j).getPrice();
          }
        }
        System.out.println(
          uniqueCategoryList.get(i) +
          " : " +
          ((categoryExpense / totalExpense) * 100) +
          " % "
        );
      }
    } catch (IOException e) {
